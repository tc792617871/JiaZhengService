...
$().ready(function() {
	...
	var specificationIndex = ${(product.specifications?size)!"0"};
	...
	<form id="inputForm" action="update.jhtml" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" id="type" name="type" value="prompt">
				...
				<td>
					<select id="productCategoryId" name="productCategoryId">
					    [#list productCategoryTree as productCategory]
							<option value="${productCategory.id}"[#if productCategory.id == product.productCategory] selected="selected"[/#if]>
			...
			<tr>
				...
				<td>
					[#if product.tagIds]
						[#list tags as tag]
							<label>
								<input type="checkbox" name="tagIds" value="${tag.id}"[#if product.tagIds?seq_contains(tag.id)] checked="checked"[/#if] />${tag.name}
							</label>
						[/#list]
					[#else]
						[#list tags as tag]
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" name="isMarketable" value="true" id="isMarketable" [#if product.isMarketable] checked="checked"[/#if]/>${message("Product.isMarketable")}
						<input type="hidden" name="_isMarketable" value="false" />
					</label>
					<label>
						<input type="checkbox" name="isSquare" value="true" id="isSquare" [#if product.isSquare] checked="checked"[/#if]/>${message("Product.isSquare")}
			...
			<tr>
				...
				<td>
					<textarea id="introduction_editor" name="introduction" class="editor" style="width: 100%;">${product.introduction?html}</textarea>
				</td>
			</tr>
		</table>
				...
				[#if product.specifications]
					[#list product.specifications as specification]
						<tr>
							...
							<td>
								<label>
									<input type="checkbox" name="specifications[${specification_index}].isEnable" value="true" [#if specification.isEnable] checked="checked"[/#if]/>
									<input type="hidden" name="_specifications[${specification_index}].isEnable" value="false" />
								</label>
							</td>
							<td>
								-
