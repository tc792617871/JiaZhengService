...
<title>${message("admin.admin.edit")} - MO&Co.</title>
			...
			<tr class="roles">
				...
				<td>
					<span class="fieldSet">
						[#list roles as role]
							<label>
								<input type="checkbox" name="roleIds" value="${role.id}"[#if admin.roleIds?seq_contains(role.id)] checked="checked"[/#if] />${role.name}
							</label>
						[/#list]
					</span>
				</td>
