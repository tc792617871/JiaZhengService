...
win.laydate = function(options){
    ...
    var e = win.event ? win.event : laydate.caller.arguments[0];
...
Dates.getElementByAttr = function(tag,attr,value)
{
...
// by maxu on 2015.07.24
Dates.tdSelcected = function(str){
	if(str=="" ||str==null){
		return;
	}
	var ymd = str.match(/\d+/g) || [];
    if(ymd.length = 3){
        var dTd=Dates.getElementByAttr('td','d',(~~ymd[2]));//返回data-id为123456的div集合
        if(dTd!=null && dTd.length>0){
            for(var i=0;i<dTd.length;i++){
                var tdElem = dTd[i];
                if(tdElem.getAttribute("m") == (~~ymd[1]) && tdElem.getAttribute("y") == ymd[0]){
                    Dates.addClass(tdElem,"laydate_click");
                }
            }
...
Dates.close = function(){
	...
	Dates.each(S('#laydate_table td'), function(i, elem){
            if(Dates.hasClass(elem, "laydate_click")){
            	//testNum = testNum + 1;
            	var getNewDate = Dates.getCreateDate([elem.getAttribute('y')|0, elem.getAttribute('m')|0, elem.getAttribute('d')|0]);
            	textContent = textContent + getNewDate +",";
            }
    });
	
	elementDoc.value=textContent;
...
//获取选中日期-maxu
Dates.getCreateDate = function(ymd, hide){
    var S = Dates.query, hms = Dates.hmsin;
    return getDates = Dates.parse(ymd, [hms[0].value, hms[1].value, hms[2].value]);
};
...
//                Dates.creation([this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0]);
                //update by maxu maxu@sencloud.com.cn
                if(Dates.hasClass(this,"laydate_nothis")){
                	return ;
                }
                if(Dates.hasClass(this,"laydate_click")){
                	Dates.removeClass(this,"laydate_click");
                }
                else{
    ...
    Dates.on(as.oclear, 'click', function(){
        Dates.elem[as.elemv] = '';
        //TODO
        $("#laydate_table td").each(function(){
        	Dates.removeClass(this, "laydate_click");
        });
