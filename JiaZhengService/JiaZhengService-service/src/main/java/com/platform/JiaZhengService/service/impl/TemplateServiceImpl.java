...
import com.platform.JiaZhengService.common.pojo.Template.Type;
...
public class TemplateServiceImpl extends BaseServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/jiaZhengService/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(preschoolEduXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/jiaZhengService/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File preschoolEduXmlFile = new ClassPathResource(CommonAttributes.JIAZHENGSERVICE_XML_PATH).getFile();
			Document document = new SAXReader().read(preschoolEduXmlFile);
			Element element = (Element) document.selectSingleNode("/jiaZhengService/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
